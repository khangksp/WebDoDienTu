...
function Checkout() {
  ...
  const { clearCart } = useCart(); // Sử dụng clearCart từ CartContext
  ...
  // Sử dụng sản phẩm từ location state (chỉ các mục được tích chọn)
  ...
  const placeOrder = async () => {
    ...
    try {
      ...
      try {
        await clearCart(cartItems); // Truyền cartItems để xóa các sản phẩm đã đặt hàng
      } catch (cartError) {
